...
- 支持 CODE_128、CODE_39、EAN-8/13、UPC-A/E、QR Code、Data Matrix
- 多码匹配：商品编号 → 客户料号 → 型号
- 出入库双模式，支持选择平台和数量
- 连续扫码模式：自动清空输入框，快速连续操作
- 声音提示：成功/失败播放不同音效（Web Audio API）
- 扫码枪智能检测：击键速度分析，自动识别扫码枪设备
- 多摄像头选择：支持切换前后摄像头，记住设备选择
- 今日扫码统计：入库/出库次数与数量实时更新
- 最近扫描记录：显示最近 20 条操作明细
...
- SQL 注入防护（PDO 预处理语句）
- XSS 防护（输出转义）
- 密码哈希存储（PASSWORD_DEFAULT，自动跟随 PHP 最新算法）
- 登录暴力破解防护（IP + 用户名限频）
- 会话超时自动退出
- 文件上传类型校验（MIME + 扩展名）
...
1. 备份数据库和所有文件
2. 替换所有 `.php` 文件
3. 首次访问时 `initDB()` 会自动执行 schema 升级，无需手动操作
4. 升级后旧数据自动兼容，`damaged` 字段默认为 0
