...
3. 升级后旧数据自动兼容，无需执行数据库迁移

v1.0.2 新增功能：

- 二维码自动识别修复（移除 experimentalFeatures，恢复 70% 扫描区域，解决 QR 码必须手动截图识别的问题）
- 扫码结果弹窗：每次识别成功弹出模态窗口显示商品编号/型号/数量变化/变化前/变化后，弹窗期间摄像头暂停识别，关闭后自动恢复，防止频繁快速识别
- 弹窗支持撤销操作：可在弹窗中直接撤销本次扫码
- 扫码页记录限制：仅显示最近 10 条记录，完整记录在出入库记录页查看
- 出入库记录页增强：新增 CSV 数据导出功能、单条记录删除、批量选择删除
- 手机端排版优化：响应式 CSS 适配小屏幕，统计卡片 2 列布局，表单单列布局
- 移动端摄像头默认后置摄像头选择
- 平台 URL 用户隔离：platforms 表新增 user\_id 字段，子用户继承父管理员平台配置
- 子用户权限新增 can\_scan、can\_print 细粒度控制
- 非主管理员默认访问平台管理页，可管理平台 URL、子用户、备份、日志
- change\_type 字段从 ENUM 改为 VARCHAR(30)，防止数据截断错误
- 二维码打印格式标准化：包含商品编号、型号、库存数量、库存变化信息
- BOM 出库处理增强：智能表头检测、合并单元格前向填充、多级匹配（编号→型号→跨平台编号）
